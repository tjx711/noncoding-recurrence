...
def calRecurrencyByChrom(mutlst,wsize):
	...
	for mut in mutlst:
		...
		if mcord == 243186095:
			print "found it: 243186095", w_start, w_end
		...
		while i < len(mutlst):
			...
			if pcord >= w_start and pcord <= w_end:
				...
				if first == 0:
					j = i
					first = 1
				## Here we summed by the mutation sample frequencies from COSMIC
				#sum_recur = sum_recur + int(mutlst[i][1])

				## Now we try - just add the number of mutations within the window
				sum_recur += 1
...
mut_dat = "data/cosmic_noncoding_mut_reg201.recur.tsv"
out_dat = "result/cosmic_noncoding_mut_reg331.100bp.rawcounts.tsv"
win_size = 100
