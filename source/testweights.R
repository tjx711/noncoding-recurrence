...
myres <- glm.nb(counts ~ ., data = mydata, weights = rep(1/N, N))

print(myres)

newweights <- c(rep(1/(2*N), N/2),
                rep(1/N, N/2))

newweights <- newweights/sum(newweights)

myres_weighted <- glm.nb(counts ~ ., data = mydata, weights = newweights)

print(myres_weighted)
